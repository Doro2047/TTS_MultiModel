"""VoxCPM LoRA 微调训练模块（现代重构版）。

⚠️ 双轨架构警告（Q4-T9 / 2026-09-05）：
    本目录是**现代重构版**训练模块，设计完整但**当前实际训练路径未接线**。
    实际训练脚本 ``scripts/train_voxcpm_finetune.py`` 导入的是 vendored 版
    ``voxcpm.training``（``app/integrated_app/vendor/voxcpm/training/``），
    而非本模块。两套模块文件同名但实现不同。
    本模块的单元测试验证其内部逻辑，不代表实际训练路径使用本模块。
    收敛方向：将训练脚本切换到本模块，或删除本模块避免双份维护。
